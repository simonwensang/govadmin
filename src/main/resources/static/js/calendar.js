...
function drawCld(SY, SM) {
    ...
    for (i = 0; i < 42; i++) {
        ...
        if (sD > -1 && sD < cld.length) {
            var gObj = document.getElementById('GD' + i);
            // gObj.setAttribute('date', SY + ((SM + 1) < 10 ? '0' + (SM + 1) : (SM + 1)) + ((sD + 1) < 10 ? '0' + (sD + 1) : (sD + 1)) + '');
            gObj.setAttribute('data-y', SY);
            gObj.setAttribute('data-m', SM + 1);
            gObj.setAttribute('data-d', sD + 1);
            sObj.innerHTML = sD + 1;
            if (cld[sD].isToday) {
